...
from agents.count_based import LSTM_DQN_Episodic_Count_Agent
...
def train(agent, env, max_num_moves, gamma=0.90):
  ...
  for _ in range(num_episodes):
    ...
    while num_moves < max_num_moves and not done:
      ...
      q_vals, discovery_bonus = agent.get_qvals(obs, infos, score)
      action, action_qval = agent.act(obs, infos, score, q_vals, eps_greedy=True, epsilon=max(1 / ((1 + tot_moves) ** 0.5), 0.15))
      print(discovery_bonus)
      ...
      next_qvals, _ = agent.get_qvals(obs, infos, score + discovery_bonus, eval_mode=True)

      if score > 0: next_action_qval = torch.tensor(0)
      else: next_action, next_action_qval = agent.get_max_qval(next_qvals, infos)

      loss = agent.loss_fcn(action_qval, score + discovery_bonus + gamma * next_action_qval)
      loss.backward()
      agent.optimizer.step()
      agent.optimizer.zero_grad()


    print('END', action, done, num_moves)

    test_scores, test_num_moves = test_on_train_level(agent, max_num_moves)
    # goal_idx = agent.action_indices.get('take coin', 0)
    # goal_qval = next_qvals[goal_idx] if goal_idx != 0 else torch.tensor(0)

    goal_value_per_epoch.append(0)
...
def test_on_train_level(agent, max_num_moves):
  ...
  for env in [TRAIN_ENV, TEST_ENV]:
    ...
    while num_moves < max_num_moves and not done:
      q_vals, _ = agent.get_qvals(obs, infos, score, eval_mode=True)
...
for i in range(NUM_ABLATIONS):
  print('ablation', i)
  untrained_baseline = LSTM_DQN_Episodic_Count_Agent(vocab_size=10000, max_num_actions=10)
...
with open('test_lstm_dqn_data.pkl', 'wb') as f:
