...
def train(agent, env, max_num_moves, gamma=0.90):
  ...
  for _ in range(num_episodes):
    ...
    while num_moves < max_num_moves and not done:
      ...
      num_options = len(infos['admissible_commands'])
      ...
      loss.backward()
      # torch.nn.utils.clip_grad_norm_(agent.model.parameters(), max_norm=1)
    ...
    goal_value_per_epoch.append(abs(goal_qval.item()))
...
def plot_test_on_train(goal_values, test_scores, num_moves, max_num_moves):
  ...
  plt.tight_layout()
  plt.savefig('LSTM_PENALTY_DQN.png')
  plt.show()
...
with open('lstm_dqn_penalty_data.pkl', 'wb') as f:
